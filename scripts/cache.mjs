...
writeFileSync(
  "dist/sw.js",
  `const CACHE='dcit418-security-lab-${revision}';
const ASSETS=${JSON.stringify(files.map((f) => "./" + f))};
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(ASSETS)).then(()=>self.skipWaiting()));});
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>(k.startsWith('dcit418-security-lab-')||k.startsWith('management-lab-')||k.startsWith('compiler-lab-'))&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));});
