Cryptography Master Exam Review (100 Questions)
Complete Ordered 100-Problem Question Bank with Verified Answers & Scannable Explanations
Total Questions: 100 Accuracy: 100% Verified Standard: Hyphenated Fill-Ins & Core MCQs
...
Explanation: AES operates on 8-bit bytes defined as elements in GF(2^8). SubBytes uses multiplicative inverses in GF(2^8),
and MixColumns performs polynomial matrix multiplication over GF(2^8).
2. The theorem guaranteeing that every integer greater than 1 factors into primes in exactly one way is called
the ____ Theorem of Arithmetic.
Correct Answer: fundamental-theorem
Explanation: The Fundamental Theorem of Arithmetic states that every integer greater than 1 has a unique prime factorization
...
algebraic structure called an ____ group.
Correct Answer: abelian
Explanation: Elliptic curve point addition satisfies closure, associativity, identity, inverses, and commutativity, forming an abelian
group.
4. The square-and-multiply technique for RSA is valuable because it computes a large modular
exponentiation such as x^16 using:
...
exponent's worth
Explanation: Square-and-multiply computes modular exponentiation in O(log e) operations using repeated squarings and
...
repeating is called a ____ of p.
Correct Answer: primitive-root
Explanation: A primitive root modulo p generates all p - 1 non-zero elements in Z_p^*.
• 
• 
Page 1 of 20
...
Explanation: SubBytes, ShiftRows, and MixColumns do not use key material; AddRoundKey is the only round operation that
...
Explanation: Arithmetic modulo n maps all results into the standard complete set of residues {0, 1, ..., n-1}.
8. The attack that defeats Double DES by working forward from the plaintext and backward from the
ciphertext to find a matching intermediate value is called the ____ attack.
Correct Answer: meet-in-the-middle
Explanation: Meet-in-the-middle matches forward encryption E_K1(P) with backward decryption D_K2(C) in an intermediate
table, reducing effective security to ~2^57.
9. The problem of finding the exponent x that solves g^x ≡ b (mod p), believed to be computationally
infeasible for large p, is called the ____ problem.
Correct Answer: discrete-logarithm
Explanation: The Discrete Logarithm Problem (DLP) is the one-way hard mathematical problem underlying classical Diffie-
Hellman and ElGamal.
...
Explanation: The final round consists only of SubBytes, ShiftRows, and AddRoundKey, omitting MixColumns to make
decryption structurally symmetrical
11. In RSA key generation, once primes p and q have been chosen, the public modulus is computed as n =
p*q and the totient is computed as:
A. phi(n) = p*q - 1
B. phi(n) = (p-1)(q-1)
C. phi(n) = p + q - 1
D. phi(n) = (p-1) + (q-1)
Correct Answer: B. phi(n) = (p-1)(q-1)
Explanation: Euler's totient function for the product of two distinct primes p and q is phi(n) = phi(p)*phi(q) = (p-1)(q-1).
12. The fast, widely used probabilistic primality test that repeatedly tests random witnesses against a
candidate number is called the ____ test.
Correct Answer: Miller-Rabin
Explanation: The Miller-Rabin test checks whether candidate witnesses satisfy modular square root properties of primes.
...
Explanation: ECC provides equivalent cryptographic security with much smaller keys (e.g., 256-bit ECC matches 3072-bit
RSA), saving compute and bandwidth.
14. In a stream cipher, the pseudorandom sequence of bits combined with the plaintext via XOR to produce
ciphertext is called the ____.
Correct Answer: keystream
Explanation: Stream ciphers generate a pseudorandom keystream that is XORed bitwise with the plaintext.
15. In GF(2^m), addition of two field elements is performed using the bitwise operation ____.
Correct Answer: XOR
Explanation: Polynomial addition over GF(2) corresponds to coefficient addition modulo 2, which is equivalent to bitwise XOR.
...
Explanation: Public-key cryptography solved key exchange over insecure channels and created digital signature mechanisms.
Page 3 of 20
...
adequate diffusion
Explanation: Skipping MixColumns in the last round makes the inverse cipher structurally identical to the forward cipher without
...
cipher becomes more vulnerable because:
...
Explanation: Too few rounds leave statistical differentials and linear paths detectable, making cryptanalysis faster than brute
force.
...
each round
Explanation: BBS evaluates x_{i+1} = x_i^2 mod n (n = p*q) and outputs the least significant bit (or parity bit) at each step.
...
Explanation: The 16 bytes of a 128-bit block are arranged in a 4-row by 4-column column-major matrix.
21. The AES round transformation that performs a non-linear byte-by-byte substitution using a fixed lookup
table is called ____.
Correct Answer: SubBytes
Explanation: SubBytes uses an S-box lookup based on GF(2^8) inversion and an affine mapping to provide non-linear
confusion.

Page 4 of 20
22. A deterministic algorithm that takes a seed and produces a long sequence of numbers that statistically
resemble true randomness is called a ____.
Correct Answer: pseudorandom-number-generator
Explanation: A Pseudorandom Number Generator (PRNG) deterministically expands a short random seed into a longer
...
(associativity, commutativity, distributivity) hold
Explanation: A field is an algebraic structure where addition and multiplication are commutative and every non-zero element
...
string
Explanation: Elements of GF(2^m) are polynomials of degree <= m - 1 with binary coefficients {0, 1}.
25. The efficient method that finds the greatest common divisor of two integers through repeated division is
called the ____ Algorithm.
Correct Answer: Euclidean
Explanation: The Euclidean Algorithm finds gcd(a, b) in logarithmic time by repeatedly taking division remainders.
26. The block cipher mode that XORs each plaintext block with the previous ciphertext block before
encryption, requiring an initialization vector for the first block, is called ____ mode.
Correct Answer: Cipher-Block-Chaining
Explanation: Cipher Block Chaining (CBC) encrypts C_i = E_K(P_i XOR C_{i-1}) with C_0 = IV.
...
Explanation: RSA is named after its creators: Ron Rivest, Adi Shamir, and Leonard Adleman.
• 
• 
Page 5 of 20
...
D. 0
Correct Answer: C. 1
Explanation: Fermat's Little Theorem states that a^{p-1} = 1 mod p for prime p coprime to a. Here 3^6 = 1 mod 7.
29. The block cipher mode specifically designed for encrypting data on sector-based block storage devices
(data-at-rest) is called ____.
Correct Answer: XTS-AES
Explanation: XTS-AES is the tweakable block cipher standard designed for sector-level storage encryption.
30. Feistel's design goal was to approximate the security of an ideal block cipher while avoiding its main
practical drawback, which is that an ideal cipher with an n-bit block requires:
...
Explanation: An ideal block cipher requires specifying an arbitrary permutation from (2^n)! possibilities, requiring an impractical
key length of ~n*2^n bits.
...
repeating
Explanation: A primitive root generates the complete set of non-zero residues modulo p.
...
Explanation: A prime number is an integer greater than 1 with exactly two distinct positive divisors: 1 and itself.

Page 6 of 20
33. The value that provides variability so that identical plaintext blocks do not produce identical ciphertext at
the start of CBC or CFB encryption is called the ____.
Correct Answer: initialization-vector
Explanation: The Initialization Vector (IV) provides uniqueness and randomness to prevent identical ciphertexts under the same
key.
...
Explanation: Stream ciphers encrypt by XORing plaintext bits directly with pseudorandom keystream bits.
...
Explanation: a = b mod n implies n divides (a - b) without remainder.
...
Explanation: XTS-AES encrypts the sector address as a tweak value to make ciphertext dependent on disk physical locations.
...
Correct Answer: 16
Explanation: Standard DES executes 16 Feistel rounds between the Initial Permutation and Inverse Initial Permutation.
38. A message encrypted only with the sender's private key, as in a basic digital signature scheme, provides
authentication but NOT confidentiality because:
...
Explanation: Because the corresponding public key is openly available, any observer can decrypt and read the message.
...
addition
Explanation: ECDLP is the challenge of determining scalar integer k given base point P and scalar multiple Q = kP.
40. The recommended padding scheme that randomizes RSA plaintext before encryption, defending against
chosen-ciphertext attacks, is called ____.
Correct Answer: OAEP
Explanation: Optimal Asymmetric Encryption Padding (OAEP) provides semantic security and chosen-ciphertext resistance for
RSA.
...
Explanation: Classified S-box design criteria led to suspicions of trapdoors, though they were actually optimized against
...
moduli
Explanation: Euler's Totient Theorem generalizes Fermat's Little Theorem from prime moduli to arbitrary composite moduli.
43. Claude Shannon's principle by which each plaintext digit affects many digits of the ciphertext, hiding
statistical structure, is called ____.
Correct Answer: diffusion
Explanation: Diffusion spreads single-bit plaintext differences across multiple ciphertext output bits.
...
Explanation: WEP concatenated 24-bit predictable IVs with the key, causing keystream reuse and enabling FMS key recovery.
...
Explanation: Public-key cryptography is computationally slow, so symmetric encryption remains essential for bulk data
encryption.
...
(periodically reseed with fresh entropy)
Explanation: NIST SP 800-90A CTR_DRBG defines instantiation (initialize), generate (produce pseudorandom output), and
reseed/update.
47. For an elliptic curve defined over the real numbers as y^2 = x^3 + ax + b, adding two distinct points P and
Q geometrically involves:
...
that point across the x-axis
Explanation: The chord-and-tangent rule finds the third intersection point R of line PQ with the curve and reflects it across the
x-axis.
48. The widely deployed stream cipher designed by Ron Rivest in 1987, later used in SSL/TLS and WEP, is
called ____.
Correct Answer: RC4
Explanation: RC4 (Rivest Cipher 4) is the byte-oriented stream cipher designed by Ron Rivest.
...
polynomial
Explanation: GF(2^m) multiplication multiplies binary polynomials and reduces the result modulo an irreducible polynomial of
degree m.
...
Explanation: The KSA initializes state S[0..255] and performs 256 key-dependent swaps.
...
inverted
Explanation: SAC requires that inverting any single input bit flips each output bit with independent probability 0.5.
...
another
Explanation: CRT guarantees a unique integer solution modulo the product of pairwise coprime moduli.
53. In RSA, the public key is the pair {e, n} and the private key is the pair {____, n}.
Correct Answer: d
Explanation: The private decryption key pair in RSA is {d, n} where d = e^{-1} mod phi(n).
Page 10 of 20
...
ciphertext with a random factor before exponentiating
Explanation: Blinding multiplies ciphertext by r^e mod n prior to decryption, removing data-dependent timing signatures.
...
Explanation: X1 = (5*7 + 3) mod 16 = 38 mod 16 = 6; X2 = (5*6 + 3) mod 16 = 33 mod 16 = 1.
...
underlying RSA
Explanation: BBS security reduces to the computational difficulty of calculating quadratic residues, equivalent to factoring n =
p*q.
...
D. 126
Correct Answer: B. 42
Explanation: gcd(84, 126) = 2^1 * 3^1 * 7^1 = 42.
...
Explanation: Diffusion dissipates individual plaintext bit patterns across large regions of ciphertext.
Page 11 of 20
1.  Using the Chinese Remainder Theorem to split RSA decryption into two smaller modular exponentiations,
one modulo p and one modulo q, speeds up decryption by roughly a factor of ____.
Correct Answer: 4
Explanation: CRT modular exponentiations are on half-sized moduli with cubic-complexity multiplication, yielding ~4x speedup.
...
Explanation: In a Feistel round, R_i = L_{i-1} XOR F(R_{i-1}, K_i).
...
Explanation: 8 mod 3 = 2 and 8 mod 5 = 3; 8 is the unique integer solution in [0, 14].
1.  The block cipher mode that turns a block cipher into a stream cipher by encrypting a counter value for
each block and XORing it with the plaintext is called ____ mode
Correct Answer: Counter
Explanation: Counter (CTR) mode turns a block cipher into a stream cipher by encrypting sequential counter values.
...
commutativity
Explanation: Under chord-and-tangent addition with point at infinity O, elliptic curve points form an abelian group.
...
extra information
Explanation: A trapdoor function is easy forward, hard to invert, but efficiently invertible with special trapdoor knowledge.
Page 12 of 20
...
ciphertext
Explanation: The avalanche effect ensures a 1-bit input flip alters approximately 50% of the ciphertext bits.
1.  AES was selected by NIST through an open competition, with the winning algorithm originally named
____, designed by Daemen and Rijmen.
Correct Answer: Rijndael
Explanation: Rijndael, created by Vincent Rijmen and Joan Daemen, was standardized as AES.
...
while relaying and altering messages
...
shared keys with each side while relaying and altering messages
Explanation: Unauthenticated Diffie-Hellman does not verify sender identities, allowing active adversaries to intercept and
negotiate split keys.
...
Explanation: Meet-in-the-middle attacks find table collisions between forward and reverse encryptions in ~2^57 steps.
Page 13 of 20
...
Explanation: The E-box expands 32 bits to 48 bits to match the 48-bit subkey for XORing before substitution in the 8 S-boxes.
1.  The block cipher mode in which each plaintext block is encrypted independently with the same key, and
which reveals repeated patterns in the plaintext, is called ____ mode.
Correct Answer: Electronic-Codebook
Explanation: Electronic Codebook (ECB) encrypts each block in isolation, leaking statistical patterns.
1.  If plaintext bit P = 1 is XORed with keystream bit K = 0, and the resulting ciphertext bit C is later XORed
again with the same keystream bit K, the result is:
...
Explanation: C = 1 XOR 0 = 1; C XOR K = 1 XOR 0 = 1, restoring P since P XOR K XOR K = P.
...
q, then recombining the results
Explanation: RSA-CRT evaluates C^{d mod p-1} mod p and C^{d mod q-1} mod q separately, recombining via CRT.
...
entries to output one byte at a time
Explanation: PRGA updates pointer indices i and j, swaps S[i] and S[j], and outputs S[(S[i]+S[j]) mod 256] per loop.
Page 14 of 20
1.  A network structure built from alternating layers of substitution and permutation across multiple rounds,
as proposed by Shannon and used in ciphers such as DES, is called a substitution-____ network.
Correct Answer: permutation
Explanation: A Substitution-Permutation Network (SPN) alternates S-boxes (confusion) and P-boxes (diffusion).
1.  Which mode of operation is generally considered to have no safe use case for general-purpose
encryption of typical application data?
A. CTR
B. ECB
C. CBC
D. OFB
Correct Answer: B. ECB
Explanation: ECB mode encrypts identical plaintext blocks to identical ciphertext blocks, failing semantic security.
...
without factoring n
Explanation: Without padding, small exponent RSA (e=3) is vulnerable to Håstad's broadcast attack via CRT and direct root
extraction.
1.  When selecting a mode of operation for high-throughput, parallelizable, general-purpose encryption, the
most appropriate recommendation is typically:
A. ECB mode
...
Explanation: CTR keystream generation depends solely on the block index counter, enabling full parallelization across CPU
cores.
...
single block
Explanation: A mode of operation defines algorithms and feedback chaining to securely encrypt sequences of arbitrary length.
Page 15 of 20
...
Explanation: For x |-> x^e mod n to be an invertible permutation over Z_n^*, e must be coprime to phi(n).
1.  Given public parameters q = 353 and alpha = 3, with Alice's private key XA = 97 and public key YA = 40,
and Bob's private key XB = 233 and public key YB = 248, Alice computes the shared secret as:
...
Explanation: In DH, Alice computes the shared secret as K = Y_B^{X_A} mod q = 248^97 mod 353.
...
first block)
Explanation: In CBC mode, Input_i = P_i XOR C_{i-1} (with C_0 = IV).
...
constant
Explanation: The AES key schedule g(w) routine applies RotWord (cyclic shift), SubWord (S-box), and XOR with Rcon.
...
Explanation: Blowfish is a 64-bit cipher designed in 1993. The 5 AES finalists were Rijndael, Serpent, Twofish, MARS, and
RC6.
Page 16 of 20
...
Explanation: NIST hosted an open multi-round international competition with transparent public cryptanalysis.
...
pseudorandom output
Explanation: Micali-Schnorr splits x_i^e mod n into pseudorandom output bits and seed feedback for the next stage.
...
Explanation: Larger blocks improve diffusion and resist birthday/dictionary attacks, but increase register size and compute
cycles.
1.  Triple DES (3DES), typically applying Encrypt-Decrypt-Encrypt with two or three keys, achieves an
effective key length of up to:
...
Explanation: 3DES using 3 independent 56-bit DES keys has a nominal total key length of 3 * 56 = 168 bits.
...
without ever transmitting the secret key itself
Explanation: Both sides exchange only modular exponentiations alpha^X mod q, computing the shared secret independently.
Page 17 of 20
...
the PRNG's speed and reproducibility
Explanation: A physical TRNG harvests hardware entropy to seed/reseed a high-throughput deterministic PRNG.
...
Explanation: The Prime Number Theorem establishes that prime density near integer n is asymptotically 1 / ln(n).
1.  The Intel Digital Random Number Generator (DRNG), used in Intel processors since 2012, combines
which stages in its pipeline?
...
exposed through the RDRAND instruction
Explanation: Intel DRNG features a thermal noise entropy source latch, CBC-MAC conditioner, and AES-CTR DRBG engine.
...
Explanation: SubBytes uses inversion in GF(2^8) to provide the cipher's essential non-linear confusion.
...
the square root
Explanation: If n = a * b, both factors cannot exceed sqrt(n); at least one factor must be <= sqrt(n).
Page 18 of 20
1.  A mode of operation that allows random access to any individual block without first decrypting all
preceding blocks is best exemplified by:
...
Explanation: CTR keystream blocks are computed directly from block counter indices, allowing instant random access.
...
reverse order
Explanation: Feistel networks decrypt using the identical forward circuit simply by reversing the subkey schedule.
...
Explanation: Modulus q and generator alpha are public parameters known to everyone.
...
Explanation: TRNGs harvest nondeterministic physical noise, whereas PRNGs apply deterministic mathematical algorithms to
expand seeds.
...
Explanation: There are 2^n possible input states, and any reversible mapping is a permutation of these states, yielding (2^n)!
possible bijections.
Page 19 of 20
1.   The primary advantage of asymmetric key cryptography over symmetric key cryptography in large
communication networks is that:
...
Explanation: In a network of N users, symmetric encryption requires N*(N-1)/2 pre-shared secret keys, whereas public-key
cryptography requires only N key pairs.
• 
Page 20 of 20